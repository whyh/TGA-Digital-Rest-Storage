...
def positive_test(keys: List[Key], *, value: Value = "v") -> None:
    ...
    response, execution_time = timed(client.get)("/bulk/items", json=keys)
    assert response.status_code == 200
    assert response.headers.get("content-type") == "application/json"
    assert response.json() == {key: value for key in keys}
...
def test_positive() -> None:
    ...
    response, execution_time = timed(client.get)("/bulk/items", json=["invalid_key"])
    assert response.status_code == 200
    assert response.headers.get("content-type") == "application/json"
    assert response.json() == {"invalid_key": None}
    assert 0.1 > execution_time

    response, execution_time = timed(client.get)("/bulk/items", json=["invalid_key1", "test_key"])
    assert response.status_code == 200
    assert response.headers.get("content-type") == "application/json"
...
def test_negative() -> None:
    ...
    response, execution_time = timed(client.get)("/bulk/items", json="not a list")
    assert response.status_code == 422
    assert response.headers.get("content-type") == "application/json"
